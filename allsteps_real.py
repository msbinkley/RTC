...
def prepare_perm_file_real(paramDict, outfilename,prep_uuidstr):  
    ...
    fileIN = open(paramDict["tmpSmall"] + "/GTExExpfile_realeQTL" +prep_uuidstr +".bed")
    causalvar = open("besteQTL_var" +prep_uuidstr +".txt")
    fileIN.readline()
    fileOUT = open(outfilename, "w")
    for j in causalvar: 
        j = j.rstrip().split('\t')
...
def get_bed_real(chrNum, pos, geneName, tissue, paramDict,prep_uuidstr):
    import deprecated_funcs as DF
    import eqtl_funcs as EF  
...
#print(residuals)
    ...
    output_bed_real(indivVector2, expV2, paramDict["tmpSmall"] + "/GTExExpfile_realeQTL" +prep_uuidstr +".bed", geneName, paramDict, chrNum, pos, tissue)
