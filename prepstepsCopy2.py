...
def output_selected_coldspots(outfilename, paramDict, geneName, chrNum, pos, tissue, snp, pvalue):
    fileIN = open ('eQTLcoldspot.txt', paramDict, geneName, chrNum, pos, tissue)
...
def find_variant_coldspot_gwas(outfilename, paramDict, geneName, chrNum, pos, tissue, snp, pvalue):
    ...
    #fileIN = open(paramDict["gtexDir"] +'/RTCperm_int5_null_gtex_oncoarray_breast.txt')
    ...
    #fileIN = open(paramDict["gtexDir"] +'/RTCperm_int4_null2_gtex_oncoarray_breast.txt')
    
    fileIN.readline()
    fileOUT = open(outfilename, "w")

    cspots = find_variant_coldspot('eQTLcoldspot.txt', paramDict, geneName, chrNum, pos, tissue, snp, pvalue)
    ...
    for i in fileIN:
        ...
        #gene = str(i[2])[:-4]
        #chrnum = str(i[0])
        #position = float(int(i[1]))
        #pvalue = .00000000000001 #just giving fake pvalue 
...
def sort_combined_gwas(outfilename, paramDict, geneName, chrNum, pos, tissue, snp, pvalue):
    #fileIN = open("gwasColdSpot.txt")
    GWAScoldspots = find_variant_coldspot_gwas("gwasColdSpot.txt", paramDict, geneName, chrNum, pos, tissue, snp, pvalue)
    sortedData = list()
    fileOUT = open(outfilename, "w")
    #for i in fileIN:
    sortedlist = list()
...
def output_selected_coldspots(outfilename, paramDict, geneName, chrNum, pos, tissue, snp, pvalue):
    ...
    coldspots = find_variant_coldspot('eQTLcoldspot.txt', paramDict, geneName, chrNum, pos, tissue, snp, pvalue)
    fileOUT = open(outfilename, "w")
...
def filter_vcf(outfilename, paramDict, geneName, chrNum, pos, tissue, snp, pvalue): 
    ...
    fileIN = open(paramDict["gtexDir"] + 'GTEx_Analysis_v7_WGS.vcf')
   
    #selectedcoldspots2 = output_selected_coldspots('preVCF.txt') #already commented out
    selectedcoldspots2 = find_variant_coldspot('eQTLcoldspot.txt', paramDict, geneName, chrNum, pos, tissue, snp, pvalue)
    fileOUT = open(outfilename, "w")
